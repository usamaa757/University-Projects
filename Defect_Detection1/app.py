...
Y_WEIGHTS = os.path.join(
    MODEL_DIR, app.config.get("YOLO_WEIGHTS", "yolov4-tiny-custom_best.weights")
)
...
def dashboard():
    analyses = (
        ImageAnalysis.query.filter_by(user_id=current_user.id)
        .order_by(ImageAnalysis.analysis_date.desc())
        .limit(20)
        .all()
    )
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            report = {
                ...
                "detection_breakdown": {
                    "classical": (
                        result.get("detection_breakdown", {}).get("classical", None)
                        if isinstance(result.get("detection_breakdown", {}), dict)
                        else None
                    ),
                    "yolo": (
                        result.get("detection_breakdown", {}).get("yolo", None)
                        if isinstance(result.get("detection_breakdown", {}), dict)
                        else None
                    ),
                    "custom": (
                        result.get("detection_breakdown", {}).get("custom", None)
                        if isinstance(result.get("detection_breakdown", {}), dict)
                        else None
                    ),
                },
...
def show_results(analysis_id):
    ...
    annotated_path = os.path.join(
        result_folder, annotated_filename
    ...
    return render_template(
        "results.html", analysis=analysis, report=report, result=result
    )
