...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_GET['quiz_id'])) {
    ...
    if (isset($_POST['answers'])) {
        ...
        // Fetch course_id for the given student_id
        $query_fetch_course = "SELECT course_id FROM student_course WHERE student_id = ?";
        $statement_fetch_course = $conn->prepare($query_fetch_course);
        $statement_fetch_course->bind_param('i', $student_id);
        $statement_fetch_course->execute();
        $result_fetch_course = $statement_fetch_course->get_result();
        $row_fetch_course = $result_fetch_course->fetch_assoc();

        if ($row_fetch_course) {
            $course_id = $row_fetch_course['course_id'];
            $course_name = $row_fetch_course['course_name'];
        }
        ...
        header("Location: view_quiz.php?course_id=" . $course_id);
        exit();
    }
}
