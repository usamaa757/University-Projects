...
class MemberManagement:
    ...
    def setup_ui(self):
        ...
        buttons = [
            ("👁️ View Details", self.view_member_details, '#3498db'),
            ...
            ("🧾 Generate Invoice", self.generate_invoice, '#1abc9c')
        ]
    ...
    def show_member_form(self, edit_mode=False, view_only=False):
        ...
        fields = [
            ...
            ("Branch:", "branch_id", "combo", self.db.get_branch_names()),  # dynamic branch names
            ("Workout Zone:", "zone_id", "combo", [])  # initially empty
        ]
        ...
        def on_branch_change(event):
            ...
            zone_names = [z[1] for z in zones]  # z[1] = zone_name
            if hasattr(self, 'zone_combo_widget'):
                self.zone_combo_widget['values'] = zone_names
                # Optionally select the first zone by default
                if zone_names:
                    self.zone_combo_widget.current(0)

        
        branch_combo_widget = scrollable_frame.grid_slaves(row=row-2, column=1)[0]  # branch combobox
        branch_combo_widget.bind("<<ComboboxSelected>>", on_branch_change)
        
        canvas.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")
        
        # Buttons (just one frame, packed)
        ...
        if not view_only:
            tk.Button(button_frame, text="Save", bg='#2ecc71', fg='white',
                    ...
                    command=lambda: self.save_member(edit_mode, form_window, zone_combo)).pack(side='left', padx=10)
    ...
    def fill_form_data(self):
        if self.current_member:
            ...
            if branch_id:
                branch_row = self.db.get_branch_by_id(branch_id)  # returns (id, name, city, contact, email)
                if branch_row:
                    branch_name = branch_row[1]  # second column is name
    ...
    def pick_date(self, var):
        # Simplified date picker (in real app, use calendar widget)
    ...
    def save_member(self, edit_mode, window, zone_combo):
        member_data = {}
        ...
        # Calculate expiry_date based on subscription_plan
        ...
        if plan == "Monthly":
            ...
            expiry_date = join_date + timedelta(days=30)  # default
    ...
    def create_initial_payment(self, cursor, member_id, member_data):
        ...
        amount = member.calculate_membership_fee()
        ...
        payment_data = {
            ...
            'final_amount': amount,
            'status': 'Completed',
            'invoice_number': f"INV-{datetime.now().strftime('%Y%m%d')}-{member_id}"
        }
    ...
    def view_attendance(self):
        ...
        # --------------------------------------------------
        # Resolve ZONE NAME FROM MEMBER TABLE (IMPORTANT)
        # --------------------------------------------------
        zone_name = "N/A"
        ...
        if member_zone_id:
            ...
            if zone_row:
                zone_name = zone_row[1]   # zone_name

        # --------------------------------------------------
        # Fetch Attendance (NO ZONE USED HERE)
        # --------------------------------------------------
        conn = self.db.get_connection()
        cursor = conn.cursor()
        cursor.execute("""
        ...
        for check_in, check_out, duration in records:
            ...
            tree.insert('', 'end', values=(
                check_in,
                check_out,
                zone_name,      # ✅ SAME ZONE FROM MEMBER TABLE
                duration
            ))
    ...
    def add_check_in(self):
        ...
        # Check if member already has an open attendance (check-out is NULL)
        cursor.execute("""
    ...
    def add_check_out(self):
        ...
        # Get the last open attendance record (check_out is NULL)
        cursor.execute("""
    ...
    def generate_invoice(self):
        ...
        ----------------------------
        Membership Type: {self.current_member['membership_type']}
        Subscription Plan: {self.current_member['subscription_plan']}
        Period: {datetime.now().strftime('%Y-%m-%d')} to {self.current_member['expiry_date']}
        
        ----------------------------
        Amount: PKR {Member(membership_type=self.current_member['membership_type'], 
                          subscription_plan=self.current_member['subscription_plan']).calculate_membership_fee():,.2f}
        ...
        # Print button
        tk.Button(invoice_window, text="Print/Save", bg='#3498db', fg='white',
