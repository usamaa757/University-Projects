...
$tests = $conn->query("
    ...
    WHERE pt.patient_id = $patient_id AND pt.doctor_id = $doctor_id
...
$care_plans = $conn->query("
    ...
    WHERE pcp.patient_id = $patient_id AND pcp.doctor_id = $doctor_id
...
<div class="container py-4">
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php
                            $last_plan_id = null;
                            while ($row = $care_plans->fetch_assoc()):
                            ?>
                        <tr>
                            <td>
                                <?php
    ...
    <div class="card mb-5 shadow-sm">
        <div class="card-header bg-secondary text-white">
            <h5 class="mb-0">Pre & Post Instructions</h5>
        </div>
        <div class="card-body">
            <?php
            $instruction_query = $conn->query("SELECT * FROM patient_instructions WHERE patient_id = $patient_id ORDER BY created_at DESC");

            if ($instruction_query && $instruction_query->num_rows > 0): ?>
            <div class="table-responsive">
                <table class="table table-bordered">
                    <thead class="table-dark">
                        <tr>
