...
if (isset($_GET['quiz_id']) && isset($_GET['student_id'])) {
    ...
    // Fetch quiz questions, options, selected answers, and correct answers
    $fetch_quiz_query = "
        SELECT 
            q.question_id, 
            q.question_text, 
        ...
        LEFT JOIN student_quiz_answers qa 
            ON q.question_id = qa.question_id 
            AND qa.student_quiz_id = (
    ...
    if (!$result_quiz) {
        echo "Error fetching quiz details: " . mysqli_error($conn);
        exit();
    }

    // Initialize an array to store quiz details
    $quiz_details = [];

    while ($row = mysqli_fetch_assoc($result_quiz)) {
        $quiz_details[$row['question_id']]['question_text'] = $row['question_text'];
        $quiz_details[$row['question_id']]['options'][] = [
            'option_id' => $row['option_id'],
            'option_text' => $row['option_text'],
            'is_correct' => $row['is_correct'],
            'selected' => ($row['selected_option_id'] == $row['option_id'])
        ];
    }
} else {
                ...
                <div class="table-responsive">
                    <?php if (!empty($quiz_details)) : ?>
                        <table width=100%>
                            ...
                            <tbody>
                                <?php foreach ($quiz_details as $question_id => $details) : ?>
                                    <tr>
                                        <td rowspan="<?php echo count($details['options']); ?>">
                                            <?php echo htmlspecialchars($details['question_text']); ?>
                                        </td>
