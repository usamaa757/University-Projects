    ...
    if ($password !== $confirm_password) {
        ...
        if (empty($errorMsg)) {
            ...
            if (empty($errorMsg)) {
                ...
                $sql = "INSERT INTO registration (student_name, student_email, gender, phone, picture, password) VALUES (?, ?, ?, ?, ?, ?)";

                // Initialize the statement
                if ($stmt = $conn->prepare($sql)) {
                    // Bind the variables to the statement
                    $stmt->bind_param("ssssss", $name, $email, $gender, $phone, $pic, $hashed_password);

                    // Execute the statement
                    if ($stmt->execute()) {
                        $resultMsg = "Registration successful!";
                    } else {
                        $errorMsg = "Error: Could not Register. " . $conn->error;
                    }
                    // Close the statement
                    $stmt->close();
                } else {
                    $errorMsg = "Error: Could not prepare the query.";
                }
            }
        }
    }
