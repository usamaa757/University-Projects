...
from .forms import LoginForm, ActivityForm, TravelTipForm, HotelDiscountForm, UserCreationForm, ChangeUserForm, MobileLoginForm, UserProfileForm, DestinationForm
from django.contrib.auth.decorators import login_required
from .models import CustomUser, Activity, Destination, TravelTip, HotelDiscount
...
def edit_profile(request):
    user_profile = CustomUser.objects.get(pk=request.user.pk)  # Fetch user by primary key
...
def create_discount(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            discount.user = request.user  # Assign the logged-in user
            discount.save()
