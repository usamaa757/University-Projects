...
if (isset($_SESSION['student_id'])) {
    ...
    $query = "SELECT sea.student_id, s.student_name, es.exam_date, es.exam_time, c.course_name, sa.seat_number 
          FROM student_exams sea
...
if ($is_single_student) {
    ...
    $pdf->Cell(40, 10, 'Seat Number', 1, 1, 'C'); // Added Seat Number column
} else {
    ...
    $pdf->Cell(35, 10, 'Exam Date', 1, 0, 'C');
    $pdf->Cell(35, 10, 'Exam Time', 1, 0, 'C');
    $pdf->Cell(60, 10, 'Subject', 1, 0, 'C');
    $pdf->Cell(30, 10, 'Seat Number', 1, 1, 'C'); // Added Seat Number column
}
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $seat_number = $row['seat_number']; // Fetching seat number
    ...
    if ($is_single_student) {
        ...
        $pdf->Cell(60, 10, $subject, 1, 0, 'C');
        $pdf->Cell(40, 10, $seat_number, 1, 1, 'C'); // Displaying seat number
    } else {
        ...
        $pdf->Cell(35, 10, $exam_date, 1, 0, 'C');
        $pdf->Cell(35, 10, $exam_time, 1, 0, 'C');
        $pdf->Cell(60, 10, $subject, 1, 0, 'C');
        $pdf->Cell(30, 10, $seat_number, 1, 1, 'C'); // Displaying seat number
    }
}
