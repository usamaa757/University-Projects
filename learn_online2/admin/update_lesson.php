...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $lesson_id = $_POST['lesson_id']; // Assuming lesson_id is passed as a hidden input
    ...
    if ($_FILES['lesson_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['lesson_file']['tmp_name'], $new_file_path)) {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: edit_lesson.php?lesson_id=$lesson_id&result=" . urlencode($resultMsg));
                exit(); // Make sure to exit after redirecting
            } else {
                $errorMsg = "Failed to update lesson details.";
                header("Location: edit_lesson.php?lesson_id=$lesson_id&error=" . urlencode($errorMsg));
                exit();
            }
            $stmt->close();
        } else {
