...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_POST['password'])) {
        ...
        if ($password !== $confirm) {
            ...
            $update_query = "UPDATE owner SET name = ?, email = ?, password = ?, phone_number = ?, address = ? WHERE owner_id = ?";
            $update_stmt = mysqli_prepare($conn, $update_query);
            mysqli_stmt_bind_param($update_stmt, "sssssi", $name, $email, $hashed_password, $phone, $address, $owner_id);
        }
    } else {
        $update_query = "UPDATE owner SET name = ?, email = ?, phone_number = ?, address = ? WHERE owner_id = ?";
        $update_stmt = mysqli_prepare($conn, $update_query);
        mysqli_stmt_bind_param($update_stmt, "ssssi", $name, $email, $phone, $address, $owner_id);
    }
