...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign'])) {
    ...
    $checkDup = $conn->query("SELECT id FROM duties WHERE exam_id = $exam_id AND user_id = $user_id");
    ...
    if ($checkDup->num_rows > 0) {
        ...
        $check = $conn->query("
            ...
            JOIN exams e2 ON e2.id = $exam_id
            WHERE d.user_id = $user_id AND e1.exam_date = e2.exam_date
        ");
        ...
        if ($check->num_rows > 0) {
            ...
            // Get exam info
            $examQ = $conn->query("SELECT exam_date, center, session FROM exams WHERE id = $exam_id");
            $examData = $examQ->fetch_assoc();

            $exam_session = strtolower(trim($examData['session']));
            $exam_center  = strtolower(trim($examData['center']));

            // Get user info
            $userQ = $conn->query("SELECT availability, center_preferences FROM users WHERE id = $user_id");
            ...
            elseif (!empty($user_preference) && strpos($user_preference, $exam_center) === false) {
                ...
                // Safe to assign
                $sql = "INSERT INTO duties (exam_id, user_id) VALUES ($exam_id, $user_id)";
...
$users = $conn->query("SELECT id, full_name, center_preferences, role, availability FROM users WHERE role IN ('superintendent','invigilator')");

// Fetch assigned duties
$assigned = $conn->query("
    SELECT d.id as duty_id, e.exam_name, e.exam_date, e.center, u.full_name, u.role, u.availability, u.center_preferences
    FROM duties d
    ...
    <form method="post">
        ...
        <select name="user_id" required>
            ...
            <option value="<?= $user['id'] ?>">
                <?= $user['full_name'] . " (" . $user['role'] . ")" . " - Availability: " . ucfirst($user['availability']) . "- Center: " . ucfirst($user['center_preferences']) ?>
            </option>
            <?php endwhile; ?>
        </select>
