...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->num_rows > 0) {
        $result = " Error: Email or CNIC already exists!";
    } else {
        // Password validation
        if (!preg_match('/^(?=.*[A-Z])(?=.*\W).{8,}$/', $password)) {
            $result = " Password must be at least 8 characters, with 1 uppercase and 1 special character.";
        } elseif (!preg_match('/^\d{13}$/', $cnic)) {
            $result = " CNIC must be exactly 13 digits.";
        } else {
            ...
            if (!empty($_FILES["profile_pic"]["name"])) {
                ...
                if (in_array($file_type, ["jpg", "jpeg", "png", "gif"])) {
                    ...
                    $result = " Only JPG, JPEG, PNG, and GIF files are allowed.";
                    $profile_pic = "";
                }
            }

            // Insert into database
            $stmt = $conn->prepare("INSERT INTO users (full_name, email, profile_pic, password, cnic, study_program, about_me) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssss", $full_name, $email, $profile_pic, $password_hash, $cnic, $study_program, $about_me);

            if ($stmt->execute()) {
                $result = " Registration successful! <a href='login.php'>Login here</a>";
            } else {
                $result = " Database Error: " . $stmt->error;
            }
            $stmt->close();
        }
    }
