...
if (!isset($_SESSION['user_id']) && !isset($_SESSION['role']) == 'jobseeker') {
...
$check = $conn->prepare("SELECT * FROM job_applications WHERE job_id = ? AND jobseeker_id = ?");
...
$stmt = $conn->prepare("INSERT INTO job_applications (job_id, jobseeker_id, resume, status) VALUES (?, ?, '', 'Pending')");
