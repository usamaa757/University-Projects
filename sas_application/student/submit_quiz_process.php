...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_GET['quiz_id'])) {
    ...
    if (isset($_POST['answers'])) {
        ...
        // Fetch subject_id for the given student_id
        $query_fetch_subject = "SELECT subject_id FROM student_subjects WHERE student_id = ?";
        $statement_fetch_subject = $conn->prepare($query_fetch_subject);
        $statement_fetch_subject->bind_param('i', $student_id);
        $statement_fetch_subject->execute();
        $result_fetch_subject = $statement_fetch_subject->get_result();
        $row_fetch_subject = $result_fetch_subject->fetch_assoc();

        if ($row_fetch_subject) {
            $subject_id = $row_fetch_subject['subject_id'];
            $subject_name = $row_fetch_subject['subject_name'];
        }
        // Insert student ID and quiz ID into student_quiz table
        $query_insert_student_quiz = "INSERT INTO student_quiz (student_id, quiz_id) VALUES (?, ?)";
        $statement_insert_student_quiz = $conn->prepare($query_insert_student_quiz);
        $statement_insert_student_quiz->bind_param('ii', $student_id, $quiz_id);
        ...
        // Redirect to a page to display the result or do further processing
        header("Location: view_quiz.php?subject_id=" . $subject_id);
        exit();
    }
}
